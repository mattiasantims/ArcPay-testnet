...
export default function CheckoutPage() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      navigate(`/payment-success?type=payment&txHash=${txHash}&proofId=${proofId}&amount=${req.amount}&merchant=${req.merchant}&ref=${encodeURIComponent(req.ref||'')}&mode=customer`)
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="fade-up" style={{ maxWidth: 520, margin: '0 auto' }}>
        ...
        <div>
          ...
          {!account ? (
            ...
            <div>
              ...
              {/* Two-step explicit UI */}
              <div style={{ marginBottom: 12 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: step === 'idle' ? 'var(--surface3)' : 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: step === 'idle' ? 'var(--text3)' : '#000', flexShrink: 0 }}>1</div>
                  <div style={{ fontSize: 12, color: step === 'idle' ? 'var(--text2)' : 'var(--green)' }}>Approve USDC spending</div>
                  {step !== 'idle' && <span style={{ fontSize: 10, color: 'var(--green)' }}>✓ Done</span>}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: step === 'paying' ? 'var(--usdc)' : 'var(--surface3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: step === 'paying' ? '#fff' : 'var(--text3)', flexShrink: 0 }}>2</div>
                  <div style={{ fontSize: 12, color: step === 'paying' ? 'var(--usdc)' : 'var(--text3)' }}>Send payment</div>
                </div>
              </div>
              <button onClick={handlePay} disabled={isLoading} className="btn-primary btn-full" style={{ marginBottom: 8, padding: '14px', fontSize: 15, minHeight: 52 }}>
                {step === 'idle'      && `✅ Pay ${req.amount} USDC`}
                {step === 'approving' && <><span className="spinner" style={{ marginRight: 8 }} />Step 1/2 — Confirm approve in MetaMask</>}
                {step === 'paying'    && <><span className="spinner" style={{ marginRight: 8 }} />Step 2/2 — Confirm payment in MetaMask</>}
              </button>
              {step === 'approving' && (
                <div style={{ background: '#f6851b18', border: '1px solid #f6851b66', borderRadius: 8, padding: '8px 12px', marginBottom: 8, fontSize: 12, color: '#f6851b', textAlign: 'center' }}>
                  🦊 Check MetaMask — approve spending cap, then return here for Step 2
                </div>
              )}
            </div>
          )}
      ...
      <div style={{ marginTop: 16, padding: 12, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 11, color: 'var(--text3)', lineHeight: 1.6 }}>
        ...
        Not a regulated payment service. No custody. Self-declared merchant data only.
      </div>
    </div>
  )
}
