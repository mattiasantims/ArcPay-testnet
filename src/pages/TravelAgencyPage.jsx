...
export default function TravelAgencyPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (p) {
        ...
        // Policy offsets are stored as days before travel/check-in.
        // Example: due 90 days before travel, deadline 75 days before travel,
        // cancellation cutoff 30 days before travel. To prefill a valid demo form,
        // choose a travel start date far enough in the future, then subtract offsets.
        const nowSec = Math.floor(Date.now() / 1000)
        const DAY = 86400
        const dueOffset = Number(p.paymentDueOffsetDays || 90)
        const deadlineOffset = Number(p.paymentDeadlineOffsetDays || 75)
        const cancelOffset = Number(p.cancellationCutoffDays || 30)
        const maxOffset = Math.max(dueOffset, deadlineOffset, cancelOffset)
        const startSec = nowSec + (maxOffset + 30) * DAY
        const dueSec = startSec - dueOffset * DAY
        const deadlineSec = startSec - deadlineOffset * DAY
        const cancelSec = startSec - cancelOffset * DAY
  ...
  function handleCreate() {
    ...
    if (allowScheduledTranche && !configured) { setError('Scheduled tranche requires ArcTravelEscrow to be configured. Disable scheduled tranche or deploy the travel contract.'); return }
    ...
    <div className="fade-up">
      <div className="card" style={{ borderColor: 'var(--green-bdr)', background: 'var(--green-bg)', textAlign: 'center', padding: '28px 24px', marginBottom: 16 }}>
        ...
        <p style={{ color: 'var(--text2)', fontSize: 14, marginBottom: 20 }}>
          {allowScheduledTranche
            ? 'Share this link with your customer. They can choose full payment or the merchant-approved scheduled tranche.'
            : 'Share this link with your customer. They can pay the full travel package amount via ArcProof.'}
        </p>
      ...
      <div className="card" style={{ marginBottom: 16, padding: 20 }}>
        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
          {allowScheduledTranche ? 'Payment schedule' : 'Payment option'}
        </div>
        {allowScheduledTranche ? (
          ...
          <div style={{ padding: 14, background: '#0a1628', border: '1px solid var(--usdc)', borderRadius: 10, textAlign: 'center' }}>
            <div style={{ fontSize: 11, color: 'var(--usdc)', marginBottom: 4 }}>Full payment via ArcProof</div>
            <div style={{ fontFamily: 'var(--display)', fontSize: 18, fontWeight: 700, color: 'var(--usdc)' }}>{total.toFixed(2)} USDC</div>
          </div>
        )}
      </div>

      <div className="card" style={{ padding: 20, marginBottom: 16 }}>
        <QRCodeBox url={travelUrl} size={160} label="QR Code — Share with customer" />
      </div>
    ...
    <div className="fade-up">
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {!configured && allowScheduledTranche && (
            <div className="error-box">
              Travel escrow contract not configured. Full payment requests can still use ArcProof, but scheduled tranche requests require deploying ArcTravelEscrow.sol and updating src/config.js.
            </div>
          )}
