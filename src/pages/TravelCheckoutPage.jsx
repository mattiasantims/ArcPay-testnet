...
export default function TravelCheckoutPage() {
  ...
  async function handlePayFull() {
    ...
    try {
      ...
      navigate(`/payment-success?type=payment&txHash=${txHash}&proofId=${proofId}&amount=${req.totalPackageAmount}&merchant=${req.merchant}&ref=${encodeURIComponent(req.travelRef||'')}&mode=customer`)
    } catch (e) {
  ...
  async function handlePayTranche() {
    if (!address || !req) return
    if (!configured) { setError('Travel escrow contract not configured. Scheduled tranche requires ArcTravelEscrow.'); return }
    setError('')
    try {
      setStep('approving')
      ...
      navigate(`/payment-success?type=travel&txHash=${txHash||''}&travelId=${travelId||''}&amount=${req.initialPaymentAmount}&merchant=${req.merchant}&ref=${encodeURIComponent(req.travelRef||'')}&mode=customer`)
    } catch (e) {
  ...
  return (
    <div className="fade-up" style={{ maxWidth: 560, margin: '0 auto' }}>
      ...
      {payMethod === 'usdc' && !payMode && (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {/* Pay initial + tranche */}
            {allowTranche && (
              <div onClick={() => configured ? setPayMode('tranche') : setError('Travel escrow contract not configured. Scheduled tranche requires ArcTravelEscrow.')} style={{ padding: '16px', borderRadius: 10, background: 'var(--green-bg)', border: '2px solid var(--green-bdr)', cursor: 'pointer' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 15, color: 'var(--green)', marginBottom: 4 }}>Pay initial + scheduled tranche</div>
                    <div style={{ fontSize: 12, color: 'var(--text2)', marginBottom: 8 }}>Initial today · Future tranche on scheduled date · Escrow protected{!configured ? ' · Travel contract not configured' : ''}</div>
      ...
      <div style={{ padding: 12, background: 'var(--surface2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 11, color: 'var(--text3)', lineHeight: 1.6 }}>
        <strong style={{ color: 'var(--text2)' }}>TESTNET ONLY.</strong> ArcPay does not advance funds. Full payment uses ArcProof. Scheduled tranche uses ArcTravelEscrow.
      </div>
    </div>
  )
}
