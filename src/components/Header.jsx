...
function WalletMenu({ address, balance, open, disconnect }) {
...
export default function Header({ balance }) {
  ...
  const reportsActive = ['/dashboard', '/booking-dashboard', '/analytics', '/merchant-profile', '/travel-dashboard'].some(r => p.startsWith(r))
      ...
      <header style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', padding: '0 24px' }}>
        <div style={{
          ...
          <nav style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <Dropdown label="Accept USDC" items={acceptItems} isActive={acceptActive} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
            ...
            {isConnected ? (
              <WalletMenu address={address} balance={balance} open={open} disconnect={disconnect} />
            ) : (
              <button
                onClick={() => open()}
