...
export async function fetchReceivedProofIds(merchantWallet) {
  try {
    // Try readContract first (fast, if supported by deployed contract)
    const pc = getPublicClient()
  ...
  } catch { /* fall through to getLogs */ }

  // Fallback: read ProofCreated events filtered by payee
  try {
    const pc = getPublicClient()
    const logs = await pc.getLogs({
      address: ARCPROOF_ADDRESS,
      event: {
        type: 'event', name: 'ProofCreated',
        inputs: [
...
export function buildReceiptObject({ proofData, txHash, proofId, merchantName, description, merchantProfile }) {
  ...
  return {
    ...
    status:           'CONFIRMED',
    disclaimer:       'TESTNET ONLY. Testnet tokens have no real economic value. This is not a financial instrument or tax document. ArcPay records self-declared merchant data and payment receipts only.',
  }
}
