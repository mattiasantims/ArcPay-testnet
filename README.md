...
Testnet MVP only. Not a production payment service or regulated escrow. ArcPay does not advance funds. Testnet USDC has no real economic value.
...
**Scheduled payments only.** ArcPay does not advance funds. The tranche must be paid by the customer when due.
...
Self-declared public merchant identity, linked wallets, and default payment/refund policy. No admin. No fees. Merchant profile data is self-declared.
...
Tabs: Overview · Luxury Retail · Online Payments · Hotel Booking · Travel Agency · Ask ArcPay AI · Coming Soon
...
ArcPay is an Arc Testnet proof of concept. Testnet USDC has no real value. ArcTravelEscrow supports merchant-approved scheduled payments only — ArcPay does not advance funds. ArcMerchantRegistry records self-declared public information only and does not verify identity or legal status. Merchant policy defaults pre-fill forms only — they do not modify existing bookings. Payment receipts are not legally valid tax invoices. Smart contracts do not auto-execute at deadlines. Not affiliated with Circle or Arc Network.
