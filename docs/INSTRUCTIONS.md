...
ArcPay is an Arc Testnet proof of concept. Testnet USDC has no real value. ArcTravelEscrow supports merchant-approved scheduled payments only. ArcMerchantRegistry records self-declared information only. Merchant policy defaults pre-fill forms only. Smart contracts do not auto-execute at deadlines. Not affiliated with Circle or Arc Network.
