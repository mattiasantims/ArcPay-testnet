...
> ArcPay is an Arc Testnet proof of concept. It is not a production payment service, regulated escrow, or compliance tool.
...
**This is scheduled payment execution only.** ArcPay does not advance funds to anyone. The agency receives funds only when the customer actually pays. The customer must have the initial payment available at booking time. The tranche must be paid by the customer when due.
...
- ArcMerchantRegistry: self-declared public information only
...
- No identity attestation, business authorisation attestation, or risk scoring
- Not a regulated payment service, regulated escrow, or consumer protection service
